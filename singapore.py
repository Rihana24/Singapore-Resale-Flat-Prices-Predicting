...
def predict_price(year,town,flat_type,flr_area_sqm,flat_model,min_storey_range,max_storey_range,
    ...
    min_storey= np.log(int(min_storey_range))
    max_storey= np.log(int(max_storey_range))
    ...
    user_data = np.array([[year_1,town_2,flt_ty_2,flr_ar_sqm_1,
                           flt_model_2,min_storey,max_storey,rem_les_year,rem_les_month,
                           lese_coms_dt]])
...
if select == "Home":
    img= Image.open(r"C:\Users\pc\Desktop\Spore_Resale\img2.jpg")
    st.image(img)

    st.header("HDB Flats:")

    st.write(''' The Housing & Development Board (HDB), is a statutory board under the Ministry of National Development responsible for the public housing in Singapore.
    ...
    st.write(''' Lessee-occupied public housing can be sold in a resale market, subject to restrictions. 
    ...
    st.write(''' To get your house valued, you can hire a professional appraiser who will assess your property's worth based on various factors such as 
             location, size, condition, and recent sales of similar properties in the area.
    ...
    st.write('''There are various housing grant schemes available to eligible buyers. Families with lower household incomes will receive a larger grant amount. 
             You can apply for the grant if your combined monthly income is $9,000 or less. The grant amount is tiered, depending on the average gross monthly 
             household income for the 12 months prior to the flat application.''')
    
    st.header("HDB Loan and Bank Loan:")

    st.write('''An HDB loan or home loan is money borrowed from HDB or the bank to help you buy your property. For HDB flats, you may also be able to tap on housing loans 
             at a concessionary interest rate, subject to HDB's criteria, while bank loans are obtained from commercial banks.''')
    ...
    with col2:
        ...
        stry_min=st.number_input("Enter the Value of Minimum Story Range (Min: 10)")

        stry_max= st.number_input("Enter the Value of Maximum Story Range(Max:50)")
    ...
    if button:
        ...
        pre_price= predict_price(year, town, flat_type, flr_area_sqm, flat_model,
                        stry_min, stry_max, re_les_year, re_les_month, les_coms_dt)

        st.write("## :green[**The Predicted Price is SGD $:**]",pre_price)
